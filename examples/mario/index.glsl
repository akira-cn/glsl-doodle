...
{
    float scanline 	= clamp( 0.95 + 0.05 * cos( 3.14 * ( uv.y + 0.008 * dd_time ) * 240.0 * 1.0 ), 0.0, 1.0 );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float resMultX  = floor( dd_resolution.x / 224.0 );
    float resMultY  = floor( dd_resolution.y / 192.0 );
    float resRcp	= 1.0 / max( min( resMultX, resMultY ), 1.0 );
    
    float time			= dd_time;
    float screenWidth	= floor( dd_resolution.x * resRcp );
    float screenHeight	= floor( dd_resolution.y * resRcp );
    ...
    vec2 uv    = fragCoord.xy / dd_resolution.xy;
